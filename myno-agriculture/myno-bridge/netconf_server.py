...
class NetconfMethods (netconf_server.NetconfMethods):
    ...
    def rpc_get (cls, unused_session, rpc, *unused_params):
        ...
        for ele in uuid_set:    
            child1 = etree.SubElement(root, 'device', xmlns=NAMESPACE+device_category[ele])
    ...
    def rpc_get_ping(self, unused_session, rpc, *unused_params):
        # unping old devices
        logger.info("Ping RPC")
        # ping()
        # time.sleep(5)
        # unping()
        ...
        #root = etree.ele("ping")
        #root.append(etree.SubElement(xmlns="urn:ietf:params:xml:ns:yang:ietf-netconf-monitoring"))  # attrib=None, nsmap=None,
